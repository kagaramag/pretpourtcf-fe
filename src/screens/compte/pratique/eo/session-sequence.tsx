...
import { Loading, Check, Icon } from "@/icons";
...
// ─── Main page ───────────────────────────────────────────────────────
export default function SpeakingSequenceSessionPage() {
  const router = useRouter();
  const params = useParams();
  const sequenceNumber = Number(params.number);
  ...
  const fetchSequence = async () => {
    try {
      ...
      const seqRes = await sequenceService.getByTypeAndNumber(
        "speaking",
        sequenceNumber
      );
      const sequence = seqRes.data.sequence;

      // Extract questions from the sequence (tâche 2 and 3 have questions)
      const selectedQuestions: PracticeQuestion[] = [];
      for (const sq of sequence.questions) {
        if (sq.tache === 2 || sq.tache === 3) {
          if (sq.questionId && typeof sq.questionId === "object") {
            selectedQuestions.push(sq.questionId as PracticeQuestion);
          }
        }
      }

      // Start a practice session using the practiceId from tâche 2
      const tache2 = sequence.questions.find((q) => q.tache === 2);
      const practiceId =
        tache2?.practiceId && typeof tache2.practiceId === "object"
          ? (tache2.practiceId as any)._id
          : tache2?.practiceId;
  ...
  const onRefresh = () => {
    ...
    // Re-fetch the sequence to start a new session
    fetchSequence();
  };
  ...
  if (loading) {
    return (
      <PracticeLayout>
        <Header
          title={`Expression orale — Série ${sequenceNumber}`}
  ...
  return (
    <PracticeLayout>
      <Header
        title={`Expression orale — Séquence ${sequenceNumber}`}
        onClose={onClose}
        onRefresh={onRefresh}
      />
      ...
      <div className="container mx-auto max-w-3xl p-4">
        ...
        <div className="flex flex-col items-center text-center space-y-6">
          ...
          {consigneListened[currentTache] &&
            ...
            activeRecordingTache !== currentTache && (
              <button
                ...
                className="group relative inline-flex items-center gap-3 rounded-full bg-red-500 px-8 py-3 text-lg font-medium text-white shadow-lg transition-all duration-300 hover:scale-103 hover:shadow-xl active:scale-103"
              >
                <span className="absolute inset-0 rounded-full bg-primary opacity-0 [animation:pulse_2s_cubic-bezier(0.4,0,0.6,1)_infinite]"></span>
                <span className="relative flex items-center gap-3 text-sm font-normal">
                  <Icon name="record" />
          ...
          {/* Recording complete for this tâche */}
