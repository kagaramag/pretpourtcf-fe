  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center">
        <h1 className="text-2xl font-bold flex-1">Mes Apprenants</h1>
        ...
        <TableBody>
          {referrals.map((referral) => (
            <TableRow
              key={referral.id}
              className={
                referral.status === "accepted" && referral.invitee
                  ? "cursor-pointer hover:bg-muted/50 transition-colors"
                  : ""
              }
              onClick={() => {
                if (referral.status === "accepted" && referral.invitee) {
                  router.push(`/trainer/apprenants/${referral.invitee.id}`);
                }
              }}
            >
              <TableCell>
